...
const userValidation = (User) => {
  ...
  if (validator.isEmpty(User.email, { ignore_whitespace: true })) {
    return { valid: false, log: `email: '${User.email}' não pode ser vazio`, statusCode: 400 }
  }
  if (!validator.isEmail(User.email)) {
    return { valid: false, log: `email: '${User.email}' precisa ser e-mail válido`, statusCode: 400 }
  }
  if (!User.password) {
